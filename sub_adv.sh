...
# Default variables
function="install"
#new version march 20
version=gemini-3h-2024-mar-20
# Options
...
[Service]
User=$USER
ExecStart=subspace-node  run --chain gemini-3h --base-path $HOME/subspace_adv --blocks-pruning 256 --state-pruning archive-canonical --farmer --name '$NODE_NAME' 
Restart=always
RestartSec=10
Nice=-5
KillSignal=SIGINT
